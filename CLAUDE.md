...
## 14. Estado real al 2026-09-25 (verificado, no de memoria)

- **Commits desde la última actualización (2026-08-05):** `516d8f0` (2026-09-04 10:38) el registro de comida acepta varios alimentos como una sola comida (`src/app/api/logging/log-meal/route.ts`, `src/features/logging/actions.ts`, nuevo esquema en `src/lib/validation/meal.ts` y pruebas en `tests/unit/validation/meal.test.ts`); `55bf6cc` (10:44) formato prettier de esos archivos; `ed7ab9a` (12:22) página pública de privacidad en `src/app/privacidad/page.tsx` y `src/middleware.ts` ajustado para dejarla pública. (La app móvil depende de estos endpoints, ver el CLAUDE.md de `nutriflowMobile`.)
- **Repo y despliegue:** GitHub `oscarjnz/nutriflow`, rama **`master`** (no `main`), HEAD `ed7ab9a` igual al local. GitHub Actions `CI` en `success` sobre `ed7ab9a` y despliegue de Vercel en `success` sobre el mismo sha. `https://nutriflow-sigma.vercel.app` responde 200. Sin cambios pendientes (solo la carpeta local `.tmp.driveupload/`, ignorable).
- **INCIDENTE ACTIVO: el backend de datos y de login no funciona.** El proyecto Supabase `nutriflow` (`drvijhxhthadzitvnitz`, us-west-2, plan free) figura `ACTIVE_HEALTHY`, pero `GET /auth/v1/health` y `GET /rest/v1/foods?limit=1` con la anon key devuelven HTTP 402: "Service for this project is restricted due to the following violations: exceed_db_size_quota". La base pesa solo 49 MB (`foods` 15 MB, `food_aliases` 14 MB, `barcodes` 6 MB, cerca de 30 mil filas cada una; sin slots de replicación ni bloat), muy por debajo del límite de 500 MB, así que el motivo NO es el tamaño de esta base. Hipótesis sin verificar en el panel: la restricción es de la organización y la dispara el proyecto `shs-app` (S.S.S.), cuyo disco está lleno (ver el CLAUDE.md de `~/proyecto s.h.s`, actualización 2026-09-25). Efecto: login y datos de la web y de la app móvil fallan mientras siga la restricción.
- **Señal temprana que ya existía:** el workflow "Supabase keep-alive" de `nutriflowMobile` falla desde el 2026-09-13 (cuatro corridas seguidas: 09-13, 09-16, 09-19, 09-22; el último `success` fue el 09-10). El log del run muestra `curl: (22) ... error: 402` contra `/auth/v1/health`. O sea, la restricción lleva al menos desde el 13 de septiembre en este proyecto.
- **Pendiente de Oscar:** resolver la restricción de la organización (ver opciones en el CLAUDE.md de S.S.S.). Cuando se levante, verificar login y `GET /rest/v1/foods` y volver a correr el índice (`node ~/.claude/scripts/project-index.mjs --remote`).
